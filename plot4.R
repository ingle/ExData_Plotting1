...
plot( strptime(paste(data$Date,data$Time), format='%d/%m/%Y %H:%M:%S'), 
     data$Global_active_power, xlab='', ylab='Global active power',type='l')
plot( strptime(paste(data$Date,data$Time), format='%d/%m/%Y %H:%M:%S'), 
     data$Voltage, xlab='datetime', ylab='Voltage',type='l')
...
legend('topright',col=c('black', 'red', 'blue'), 
       ...
       lty=c(1,1,1), seg.len=3, box.lwd=0)

plot( strptime(paste(data$Date,data$Time), format='%d/%m/%Y %H:%M:%S'), 
     data$Global_reactive_power, xlab='datetime', ylab='Global_reactive_power',type='l')
